...
def f_baseline_globalBalancing(train_or_test, X_in, Y_in, learning_rate, num_steps, tol):
    ...
    if train_or_test == 1:
        ...
        for i in range(1, num_steps + 1):
            _, l, l_predictive, l_balancing, l_regulizer, l_regulizer_l2 = sess.run(
                [optimizer, loss, loss_predictive, loss_balancing, loss_regulizer, loss_regulizer_l2],
                feed_dict={X: X_feed, Y: Y_in})
            if abs(l - l_pre) <= tol:
