...
def evaluate_bitcoin_explanation(explanations, args):
    ...
    # Get ground truth
    filename_pos = os.path.join('../Generate_XA_Data/ground_truth_explanation/' + prog_args.dataset,
                                prog_args.dataset + '_pos.csv')
    filename_neg = os.path.join('../Generate_XA_Data/ground_truth_explanation/' + prog_args.dataset,
    ...
    with open(report_file, "a") as text_file:
        text_file.write(
            args.dataset + ", " + str(args.num_perturb_samples) + " samples, " + top + " | Precision: " + str(
                precision) + "\n")
        text_file.write("\n")
...
# 根据config的信息读取对应的explaination结果
