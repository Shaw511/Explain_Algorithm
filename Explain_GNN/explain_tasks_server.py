...
import numpy as np
import torch
import pgm_explainer_server as pe
...
def task_syn(args):
    ...
    if args.explain_node == None:
        if args.dataset == 'syn1':
            explanations = explainer.explain_range(list(range(300, 325)), num_samples=args.num_perturb_samples, top_node=args.top_node)  # 原始范围是300-700
            ...
            explanations = explainer.explain_range(list(range(511, 561)), num_samples=args.num_perturb_samples, top_node=args.top_node,
                                                   pred_threshold=0.1)  # (511, 871)
        elif args.dataset == 'syn5':
            explanations = explainer.explain_range(list(range(511, 561)), num_samples=args.num_perturb_samples, top_node=args.top_node, pred_threshold=0.05)  # (511, 1231)
        elif args.dataset == 'syn6':
            explanations = explainer.explain_range(list(range(300, 350)), num_samples=args.num_perturb_samples, top_node=args.top_node) # (300, 700)
    else:
