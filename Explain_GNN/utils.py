...
def gen_filesave(args):
    ...
    num_samples = args.num_perturb_samples
    if args.top_node == None:
        top = "no_top"
    else:
        top = "top_" + str(args.top_node)
    savedir = 'result/'+ args.dataset + '/explanations_' + args.dataset + "_" + top + "_" + str(num_samples) +"_samples"+".npy"
    return savedir
...
def load_ckpt(args, isbest=False):
    ...
    if args.ckptdir is None:
        filename = create_filename("../Train_GNN_model/ckpt", args, isbest)
    else:
