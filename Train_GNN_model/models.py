...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers, pred_hidden_dims=[], concat=True,
                 bn=True, dropout=0.0, add_self=False, args=None, ):
        ...
        # concat表示是否把不同图卷积层的嵌入拼接到最终得到的embedding当中
        if concat:
    ...
    # gcn_forward应该是为node classification任务写的，对于图分类任务，直接用下面的forward函数就行
    def gcn_forward(self, x, adj, conv_first, conv_block, conv_last, embedding_mask=None):
        ...
        # 对syn1数据来说，这里并没有改变邻接矩阵，下面是自己加的判断语句
        # if np.array_equal(adj, adj_att):
        #     print("两个数组相等")
        # else:
        #     print("两个数组不相等")
        x = self.act(x)
        if self.bn:
            x = self.apply_bn(x)
        # 如果concat属性是true的话，需要用到下面x_all和adj_att_all需用用到
        x_all = [x]
        adj_att_all = [adj_att]
        # out_all = []
        ...
        x_all.append(x)
        adj_att_all.append(adj_att)
        # x_all中有3个700*20的向量，torch.cat(x_all, dim=2)后变成700*60
...
# 继承了父类中的一些内容， class GcnEncoderNode(GcnEncoderGraph):表示当前类继承了父类GcnEncoderGraph中的函数
class GcnEncoderNode(GcnEncoderGraph):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers, pred_hidden_dims=[], concat=True,
                 bn=True, dropout=0.0, args=None, ):
        super(GcnEncoderNode, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                                             pred_hidden_dims, concat, bn, dropout, args=args, )
    ...
    def forward(self, x, adj, batch_num_nodes=None, **kwargs):
        ...
        self.adj_atts = []
        self.embedding_tensor, adj_att = self.gcn_forward(x, adj, self.conv_first, self.conv_block, self.conv_last, embedding_mask)
