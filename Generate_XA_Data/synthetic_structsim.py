...
def ba(start, width, role_start=0, m=5):
    ...
    INPUT:
    -------------
    start       :    starting index for the shape，start是初始节点的编号
    width       :    int size of the graph
    role_start  :    starting index for the roles，标签的初始编号
    OUTPUT:
    -------------
    graph       :    a house shape graph, with ids beginning at start
    roles       :    list of the roles of the nodes (indexed starting at
...
# 调用格式为: synthetic_structsim.build_graph(width_basis, basis_type, list_shapes, start=0, m=5)
def build_graph(
    width_basis,
    basis_type,
    list_shapes,
    ...
    INPUT:
    --------------------------------------------------------------------------------------
    width_basis      :      width (in terms of number of nodes) of the basis, width_basis=300, 表示basis图中有300个节点
    basis_type       :      (torus, string, or cycle)
    shapes           :      list of shape list (1st arg: type of shape,
                            next args:args for building the shape,
                            except for the start)
    start            :      initial nb for the first node，初始节点的编号，
    ...
    # 本文用的是ba这种形式
    if basis_type == "ba":
        # ba这个函数是返回的是一个基准的图和图中的节点的标签，所有标签会形成一个列表role_id
        basis, role_id = eval(basis_type)(start, width_basis, m=m)
    else:
        basis, role_id = eval(basis_type)(start, width_basis)

    n_basis, n_shapes = nx.number_of_nodes(basis), len(list_shapes)
    start += n_basis  # indicator of the id of the next node, 计算house形状的初始节点

    # Sample (with replacement) where to attach the new motifs，计算在什么位置插入house形状， plugins是个列表，保存了每个house和basis的连接位置
    ...
    for shape_id, shape in enumerate(list_shapes):
        ...
        args += [0]
        # args是[300, 0]这种形式的
        # 通过下面这行代码生成一个house形状的结构，grpah_s是图结构， roles_graph_s是节点的标签集合
