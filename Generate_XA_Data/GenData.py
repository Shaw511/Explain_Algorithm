...
prog_args = configs.arg_parse()  # 该脚本被设计为接受命令行参数，并使用 configs.arg_parse() 来解析这些参数。解析后的参数存储在 prog_args 变量中。
...
if prog_args.dataset is not None:
    if prog_args.dataset == "amazon":
        ...
        # eval在这里将 generate_function 中的字符串转化为一个函数对象。
        G, labels, name = eval(generate_function)(feature_generator=featgen.ConstFeatureGen(np.ones(prog_args.input_dim, dtype=float)))
        utils.save_XAL(G, labels, prog_args)
